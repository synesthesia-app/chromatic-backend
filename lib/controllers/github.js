...
module.exports = Router()
  .get('/login', async (req, res) => {
    res.redirect(
      `https://github.com/login/oauth/authorize?client_id=${process.env.CLIENT_ID}&scope=user&redirect_uri=${process.env.API_URL}:${process.env.PORT}/api/v1/github/login/callback`
    );
  })

  .get('/login/callback', async (req, res, next) => {
    console.log('*** -19 -github.js *** req.query.code ==> ' + req.query.code);
    // try {
    const user = await UserService.create(req.query.code);
    console.log('*** -21 -github.js *** user ==> ' + user);
